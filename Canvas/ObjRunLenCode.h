...
public:
	...
	ObjRunLenCode();
	ObjRunLenCode(const CString& strFileName);
	ObjRunLenCode(ObjRunLenCode& objRLC);
	virtual ~ObjRunLenCode();

	// 序列化
	void Serial(CArchive& ar);
	// 打开文件
	...
	// 绘图
	void Draw(CDC* pDC, int nImgHeight, COLORREF clr=RGB(255,0,0));
	void SetObjColor(int nObjIdx, CDC* pDC, int nImgHeight, COLORREF clr);
	// 返回目标颜色
	...
	// 添加一个目标
	void ObjAdd(PRUNLENGTHCODE_MGR pObject);
	// 删除第nIdx个目标的指针
...
protected:
	...
	long double m_dbZoomX, m_dbZoomY;
