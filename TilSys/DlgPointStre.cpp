...
void CDlgPointStre::GrayStretch(BYTE bX1, BYTE bY1, BYTE bX2, BYTE bY2)
{
	...
	for (i = 0; i < lHeight; i ++)
	{
		for (j = 0; j < lLineWidth; j ++)
		{
			// 对像素各颜色分量进行灰度映射处理
			unsigned char R = *((unsigned char *)lpDIBBits + lLineWidth * i + j);
			*((unsigned char *)lpDIBBits + lLineWidth * i + j) = bMap[R];
			j++;
			unsigned char G = *((unsigned char *)lpDIBBits + lLineWidth * i + j);
			*((unsigned char *)lpDIBBits + lLineWidth * i + j) = bMap[G];
			j++;

			// 在此如用lWidth * 3则有彩色效果，如用lLineWidth则为普通灰度处理
			unsigned char B = *((unsigned char *)lpDIBBits + lWidth * 3 * i + j);
			*((unsigned char *)lpDIBBits + lLineWidth * i + j) = bMap[B];
		}
	}
