...
void CDragTree2::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (m_nClickNum == 1)
	{
		...
		CString strOldFilePath = pCanvas->GetFilePath();
		CString strOldFileName = pCanvas->GetFileName();

		CFileFind ff;

		// 更改.src文件名
		CString strOldPathName, strNewPathName;
		strOldPathName = strOldFilePath + '\\' + strOldFileName + ".src";
		strNewPathName = strOldFilePath + '\\' + strFileName + ".src";
		if (ff.FindFile(strOldPathName))
			CFile::Rename(strOldPathName, strNewPathName);

		// 更改.crk文件名
		strOldPathName = strOldFilePath + '\\' + strOldFileName + ".crk";
		strNewPathName = strOldFilePath + '\\' + strFileName + ".crk";
		if (ff.FindFile(strOldPathName))
			CFile::Rename(strOldPathName, strNewPathName);

		// 更改.obj文件名
		strOldPathName = strOldFilePath + '\\' + strOldFileName + ".obj";
		strNewPathName = strOldFilePath + '\\' + strFileName + ".obj";
		if (ff.FindFile(strOldPathName))
			CFile::Rename(strOldPathName, strNewPathName);

		pCanvas->SetFileName(strFileName);

		GetCanvasMgr()->SetModifiedFlag();
