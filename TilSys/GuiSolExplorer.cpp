...
void CGuiSolExplorer::FillerSolExplorer()
{
	...
	HTREEITEM hSrc = m_TreeSolExplorer.InsertItem (_T("结构名称"), 11, 12, hRoot);
...
void CGuiSolExplorer::InsertFileToSolExplorer(const CString& fileName)
{
	...
	if (hRoot != NULL)
	{
		if (hPrj == NULL)
		{
			hPrj = m_TreeSolExplorer.InsertItem(_T("结构名称"), 16, 16, hRoot);
		}
		if (hPrj != NULL)
		{
...
/*
		...
		{
			CString strFileName = pProject->GetCanvasByIdx(i)->GetFileName();
			m_TreeSolExplorer.InsertItem(strFileName, 7, 7, hPrj);
		}
	}
